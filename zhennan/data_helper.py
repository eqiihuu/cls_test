...
def encode_vds(vds_str, vds2id):
    words = vds_str.split(' ')
    tags = np.zeros(sentence_length)
    for i in range(len(words)):
        if i >= sentence_length:
            break
        vid = vds2id[words[i]]
...
#     Read data vd
def read_data_vd(feature_file, label2id, word2id, reg2id, vds2id):
    utterances = []
    vds = []
    regs = []
    y = []
    ...
    with open(feature_file, 'r') as f:
        for line in f:
            ...
            index += 1
        # print vds
    print 'Discard: %d, Used: %d' % (z, len(y))
    return utterances, vds, regs, y, index_list