...
class CNN(object):
    # Define the model
    reg = 1
    # Use the VDS feature (1 for multi-hot vector, 2 for 12D vecter)
    ...
    def __init__(self,
        ...
        with tf.device(self.device):
            ...
            # Merge words and VDS
            self.x_word_vds = tf.concat([self.embedded_word, self.x_vds], 2)
            # VDS Convolution layer
            self.x_word_vds_expanded = tf.expand_dims(self.x_word_vds, -1)  # why expanding?
            filter_shape = [filter_size, word_embed+vds_size, 1, filter_num]
            ...
            W_f = tf.Variable(tf.constant(0.0, shape=[filter_num+self.reg*reg_embed, num_class]), name='W_f')
    ...
    def train(self, dropout, check_step, save_step, batch_size, epoch_num, model_name,
        ...
        root_path = './save/reg_%d_%d/' % (self.reg, time.time())
        os.mkdir(root_path)
        curr_step = 0
