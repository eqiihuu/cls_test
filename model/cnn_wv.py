...
class CNN(object):
    ...
    def __init__(self,
        ...
        with tf.device(self.device):
            ...
            self.word_vds = tf.concat([self.pool_vds_flat, self.pool_word_flat], 1)
            W_f1 = tf.Variable(tf.constant(0.0, shape=[2 * filter_num, filter_num]),
                              name='W_f')
            b_f1 = tf.Variable(tf.constant(0.0, shape=[filter_num]), name='b_f')
            self.feature = tf.nn.xw_plus_b(self.word_vds, W_f1, b_f1, name='score')
